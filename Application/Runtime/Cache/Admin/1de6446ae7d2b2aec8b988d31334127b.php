				...
				<table id="App-table" class="table table-bordered table-hover">
					...
					<tbody>
						<?php if(is_array($cache)): $i = 0; $__LIST__ = $cache;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="item<?php echo ($vo["id"]); ?>">
								...
								<?php if(empty($buytype)): ?><td class=" "><?php switch($vo["buytype"]): case "0": ?>普通<?php break;?>
								<?php case "1": ?>团购<?php break; endswitch;?></td>
								<empty name="status">
								<td class=" "><?php switch($vo["status"]): case "0": ?>已取消<?php break;?>
