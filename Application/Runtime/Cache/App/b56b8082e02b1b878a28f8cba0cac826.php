...
<body class="index">
    ...
    <div class="zbb_index">
        <header class="ui-banner">
         ...
         <!-- 产品搜索框 -->
        <div> 
           <form id="key_search" method="post" action="<?php echo U('App/Shop/search/');?>">
            <input id="name" name="name" type="text" class="form-control" placeholder="测试阶段请勿购买"/>
                ...
                <div id="nav" style="width:100%;">
                    <ul>
                        <li <?php if(($type) == "0"): ?>class="ui-state-active"<?php endif; ?> id="ui-li">
                            <a href="<?php echo U('App/Shop/cateList',array('type'=>0));?>">
                        ...
                        <?php if(is_array($indexicons)): $i = 0; $__LIST__ = $indexicons;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li <?php if(($vo["ison"]) == "1"): ?>class="ui-state-active"<?php endif; ?> id="ui-li">
                                <a href="<?php echo ($vo["url"]); ?>">
                                    <div <?php if(($vo["ison"]) == "1"): ?>class="ui-imgs indexiconson"
                                        <?php else: ?>class="ui-imgs indexiconsoff"<?php endif; ?> style="border-radius:50%;margin-top:8px;background-image: url('<?php echo ($vo["iconurl"]); ?>');" id="ui-imgs"></div>
        ...
        <!-- 团购列表 -->
         <div class="index-alst">
           <div class="lst-tg">
           <h1>本期团购</h1><!-- <div style="border:1px solid #CCC"></div><hr/> -->
           </div>
           <div class="index-glist ovflw" id="index-glist">
                <ul class="glist-ul ovflw">
                    <?php if(is_array($gbuycache)): $i = 0; $__LIST__ = $gbuycache;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                          <div class="glidiv">
                            <a href="<?php echo U('App/Shop/groupbuy',array('sid'=>0,'id'=>$vo['id'],'ppid'=>$_SESSION['WAP']['vipid']));?>">
                                <div class="showImg-divg"><img class="showImgg" src="<?php echo ($vo["imgurl"]); ?>" /></div>
                                <!--图片尺寸：336*259-->
                                <div class="gdiv" style="margin-top: -15px;">
                                <h1 class="glist-tt fonts4 ovflw"><?php echo ($vo["name"]); ?></h1>
                                <p><span class="plist-yp color3"><i class="fonts3">团购价:￥</i><em class="fonts1"><?php echo ($vo["price"]); ?></em></span>&nbsp;<span class="plist-xp fonts3">原价:￥<?php echo ($vo["oprice"]); ?></span><br/><span>限量<?php echo ($vo["minnumber"]); ?>件&nbsp;&nbsp;</span><button id="gbtn" class="zan-btn" type="button" onclick="addnumber(<?php echo ($vo['id']); ?>)"></button><span id="add_num"><?php echo ($vo["zan"]); ?></span>
                               </p>
                               </div>
                            </a> 
        ...
        <!-- 每日 -->
        <div class="index-lst">
            <div class="lst-tt"><span class="color11 lst-mr"><?php echo ($group["name"]); ?></span>&nbsp;<span class="color2 fonts3"><?php echo ($group["summary"]); ?></span></div>
            <ul class="lst-ul">
        ...
        <div class="index-alst">
            ...
            <div class="index-alst">
           <div class="lst-tt">
               <span class="color11 lst-mr" style="margin-left:10px;margin-top: 10px;">今日上新</span>
           </div>
            <div class="index-plist ovflw">
    ...
    <script type="text/javascript">
       function addnumber(id) {
        console.log(id);
                $.post("/App/Shop/addnumber", {id:id}, function(data) {
                
                })
                var zan=$('#add_num').text();
                 zan =  parseInt(zan)+1;
                 $("#add_num").text(zan);
