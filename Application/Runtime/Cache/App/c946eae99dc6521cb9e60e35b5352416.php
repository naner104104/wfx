...
<body class="back1 color6">
		...
		<div class="ads-lst border-t1 border-b1 ovflw mr-b back2">
			...
			<p class="border-b1 ads_ortt3 fonts85 color3">&nbsp;使用代金卷<span class="fr"><select name="djqid" id="djqid" class="ads-sel"><option value="0" data-money="0">请选择有效代金卷</option><?php if(is_array($djq)): foreach($djq as $key=>$vo): ?><option value="<?php echo ($vo["id"]); ?>" data-money="<?php echo ($vo["money"]); ?>"><?php echo ($vo["money"]); ?>元代金卷</option><?php endforeach; endif; ?></select></span></p>
			<p class="border-b1 ads_ortt3 fonts3">&nbsp;邮费政策：<?php if(($isyf) == "1"): ?>全场定邮<?php echo ($yf); ?>元，订单满<?php echo ($yftop); ?>元包邮。<?php else: ?>全场包邮<?php endif; ?></p>
			<p class="border-b1 ads_ortt3 fonts3 ads"><input type="text" name="msg" class="ads_orinput" placeholder="给卖家留言"/></p>
