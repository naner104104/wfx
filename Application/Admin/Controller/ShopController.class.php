...
class ShopController extends BaseController
{
     ...
     public function groupbuy()
    {
        ...
        $cache = $m->where($map)->page($p, $psize)->select();
        $count = $m->where($map)->count();
        $this->getPage($count, $psize, 'App-loader', '团购管理', 'App-search');
        $this->assign('cache', $cache);
        $this->display();
    }
    ...
    public function groupbuySet()
    {
        ...
        $bread = array(
            ...
            '2' => array(
                'name' => '团购设置',
                'url' => $id ? U('Admin/Shop/groupbuySet', array('id' => $id)) : U('Admin/Md/mdSet'),
            ),
        );
        $this->assign('breadhtml', $this->getBread($bread));
        //处理POST提交
    ...
    public function groupbuyStatus()
    {
        $m = M('Shop_groupbuy');
        $now = I('status') ? 0 : 1;
        $map['id'] = I('id');
    ...
    public function cate()
    {
        ...
        //$cache=$m->$field->where($map)->order('sorts desc')->select();
        $this->assign('cache', $cache);
        $this->display();
    }
